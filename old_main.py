...
class View(QGraphicsView):
    ...
    def cell_occupied(self, belt: Belt, idx: int) -> bool:
        """Return True if the given belt cell index is occupied by any active box."""
        try:
            for bx in self.boxes:
    ...
    def tick(self):
        ...
        if getattr(self, 'generator', None) is not None:
            ...
            # kun je nu spawnen? (allow multiple boxes on a belt but not in the same cell)
        ...
        for bx in list(self.boxes):
            ...
            try:
                ...
                print (f"Checking belts {belt.label!r}: motor_var={belt.motor_var!r} prev={prev} motor_on={motor_on}")
                if motor_on != prev:
                    # white when ON, dark gray when OFF
                    belt.setBrush(QBrush(Qt.white if motor_on else Qt.darkGray))
                    # brief debug print to help troubleshooting
                    print(f"[belt] {belt.label!r} motor_var={belt.motor_var!r} motor_on={motor_on}")
                ...
                try:
                    ...
                    if new_cell != cur_cell:
                        ...
                        if not self.cell_occupied(belt, new_cell):
                            ...
                            # wait in current cell (do nothing)
                            pass
                    else:
    ...
    def _poll_snap7(self):
        ...
        try:
            ...
            if self._snap_client.get_connected():
                ...
                # Update global VARS from parsed DB variables so UI logic reads live values
                try:
                    # db_block.data maps variable names -> metadata; use keys to read values
                    for name in list(getattr(self.db_block, 'data', {}).keys()):
                        try:
                            val = self.db_block[name]
                            # normalize to boolean for motor/sensor flags; non-zero/truthy -> True
                            VARS[name] = bool(val)
                        except Exception:
                            # if getting the variable fails, leave existing VARS entry alone
                            continue
                except Exception:
                    pass
